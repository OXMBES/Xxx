...
"""
╔══════════════════════════════════════════════════════════════════╗
║          EXSLV — Advanced Android Pentesting Tool          ║
║          Author : HexSecTeam                                     ║
...
def print_banner():
    """Print the animated EXSLV banner with live status."""
...
def handle_report_generator():
    ...
    if fmt in ("html", "both"):
        out = Prompt.ask("[cyan]HTML output filename[/]", default="EXSLV_report.html")
        report_generator.generate_html_report(data, out)

    if fmt in ("json", "both"):
        out = Prompt.ask("[cyan]JSON output filename[/]", default="EXSLV_report.json")
...
def handle_about():
    about = Panel(
        ...
        f"  [bold magenta]YouTube:[/] [cyan]{YOUTUBE}[/]\n"
...
def interactive_mode():
    ...
    console.print(Panel(
        ...
        "[white]EXSLV is designed for authorized security testing ONLY.\n"
    ...
    while True:
        ...
        choice = Prompt.ask("\n[bold cyan]EXSLV ▶[/]", choices=valid_choices, show_choices=False)

        if choice == "0":
            console.print("\n[bold magenta]👻 Exiting EXSLV. Stay ethical.[/]\n")
            sys.exit(0)
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="EXSLV",
        description=f"👻 EXSLV v{VERSION} — Advanced Android Pentesting Tool by {AUTHOR}",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 main.py --interactive
  python3 main.py --apk app.apk --report html
  python3 main.py --device ABC123 --vuln-scan --pkg com.example.app
  python3 main.py --device ABC123 --port-scan
  python3 main.py --payload reverse_tcp --lhost 10.0.0.1 --lport 4444
  python3 main.py --device ABC123 --exploit deep-link --pkg com.example --scheme myapp
...
def cli_mode(args):
    ...
    if args.report:
        ...
        if args.report in ("html", "both"):
            out = args.report_out or "EXSLV_report.html"
            report_generator.generate_html_report(data, out)
        if args.report in ("json", "both"):
            out = args.report_out or "EXSLV_report.json"
            report_generator.generate_json_report(data, out)
...
if __name__ == "__main__":
    try:
        ...
        console.print("\n\n[bold magenta]👻 EXSLV interrupted. Stay ethical.[/]\n")
        sys.exit(0)
